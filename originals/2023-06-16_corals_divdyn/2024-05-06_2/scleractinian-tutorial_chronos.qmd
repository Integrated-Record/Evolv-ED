...
bibliography: 'scleractinian-tutorial_chronos.bib'
toc: true
author:
  - name: Ádám T. Kocsis 
...
```{r sampMerge, echo=TRUE, eval=TRUE}
...
It is recommended to merge the output of the `divDyn()` function with that of the timescale, similar to how we did it earlier.

```{r ddMerge, echo= TRUE, eval=TRUE,plot=TRUE, fig.height=5.5}
ddMerged <- merge(stages, dd, by="stg")
```
...
cr50Merged <- merge(stages, cr50, by="stg")
...
# to ensure alignment, the result is merged with the timescale
sqsDDMerged <- merge(stages, sqsDD, by="stg")

tp(ylim=c(0,100), ylab="Subsampled richness (SQS, 0.5)")	
lines(sqsDDMerged$mid, sqsDDMerged$divRT, lwd=2)
```
...
lines(sqsDDMerged$mid, sqsDDMerged$divCSIB, lwd=2)
```
...
Determining the symbotic status (whether or not they host photosymbionts) of extant corals is straightforward, but not so for fossil ones. Although the photosymbionts do not fossilize, some morphological traits and geogchemical properties correlate with the corals' ability to host symbionts. W. Kiessling compiled a list of most likely photosymiotic status for coral genera occurring in the Paleobiology Database. This list is available from the `chronosphere` and can be loaded directly with this line of code. 

```{r symtab, echo= TRUE, eval=TRUE}
sym <- fetch(src="SOM-kiessling-coralgenera", ver="0.54")
```
...
zDDMerged <- merge(stages, zDD, by="stg")
azDDMerged <- merge(stages, azDD, by="stg")
```
...
selIntervals<-cbind(zDDMerged$oriPC[rs$ori], azDDMerged$oriPC[rs$ori])
...
# display selectivity with points
# select the higher rates
selIntervals<-cbind(zDDMerged$extPC[rs$ext], azDDMerged$extPC[rs$ext])
groupSelector<-apply(selIntervals, 1, function(x) x[1]<x[2])
